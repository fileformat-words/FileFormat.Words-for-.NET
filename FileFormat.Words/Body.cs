    ...
    public class Body : Document
    {
        ...
        public List<Table.Table> getDocumentTables
        {
            ...
            get
            {
                ...
                foreach (DocumentFormat.OpenXml.Wordprocessing.Table tbl in table.getDocumentTables(this.wordDocumentBody))
                {
        ...
        public List<Table.TableCell> FindTableCell(int tableIndex, int tableRow, int tableCell)
        {
            ...
            DocumentFormat.OpenXml.Wordprocessing.Table table = tab.getDocumentTables(this.wordDocumentBody)[tableIndex];
            ...
            if (tableIndex >= tab.getDocumentTables(this.wordDocumentBody).Count())
            {
        ...
        public int FindTableByText(string text)
        {
            Table.Table table = new Table.Table();
            List<Table.Table> ls = new List<Table.Table>();
            List<DocumentFormat.OpenXml.Wordprocessing.Table> tab = table.getDocumentTablesByText(this.wordDocumentBody, text);
            return tab.Count();
        }
    }
}
